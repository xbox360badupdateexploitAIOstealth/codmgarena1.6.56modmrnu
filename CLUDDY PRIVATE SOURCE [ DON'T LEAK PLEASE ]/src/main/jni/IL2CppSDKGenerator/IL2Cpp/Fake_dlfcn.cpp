...
#define g_LogTag OBFUSCATE("Fake_Dlfcn-LOGGER")
...
#if defined(__LP64__)
static const char *const kSystemLibDir = OBFUSCATE("/system/lib64/");
static const char *const kOdmLibDir = OBFUSCATE("/odm/lib64/");
static const char *const kVendorLibDir = OBFUSCATE("/vendor/lib64/");
static const char *const kApexLibDir = OBFUSCATE("/apex/com.android.runtime/lib64/");
static const char *const kApexArtNsLibDir = OBFUSCATE("/apex/com.android.art/lib64/");
#else
static const char *const kSystemLibDir = OBFUSCATE("/system/lib/");
static const char *const kOdmLibDir = OBFUSCATE("/odm/lib/");
static const char *const kVendorLibDir = OBFUSCATE("/vendor/lib/");
static const char *const kApexLibDir = OBFUSCATE("/apex/com.android.runtime/lib/");
static const char *const kApexArtNsLibDir =  OBFUSCATE("/apex/com.android.art/lib/");
#endif
...
extern "C" {
	...
	static void *fake_dlopen_with_path(const char *libpath, int flags) {
		...
		for (k = 0; k < elf->e_shnum; k++, shoff += elf->e_shentsize) {
			...
			case SHT_DYNSYM:
				if (ctx->dynsym)
					fatal(OBFUSCATE("%s: duplicate DYNSYM sections"), libpath);
					ctx->dynsym = malloc(sh->sh_size);
				if (!ctx->dynsym)
					fatal(OBFUSCATE("%s: no memory for .dynsym"), libpath);
			...
			case SHT_STRTAB:
				...
				if (!ctx->dynstr)
					fatal(OBFUSCATE("%s: no memory for .dynstr"), libpath);
	...
	static void *fake_dlsym(void *handle, const char *name) {
		...
		if (strcmp(strings + sym->st_name, name) == 0) {
			...
			__android_log_print(ANDROID_LOG_INFO, g_LogTag, OBFUSCATE("%s found at %p"), name, ret);
			return ret;
		}
		return 0;
	}
