...
void *Il2CppGetImageByName(const char *image) {
	void *assemblies = il2cpp_assembly_get_image(il2cpp_domain_assembly_open(il2cpp_domain_get(), image));
	if (assemblies)
		return assemblies;
	return 0;
}
...
void *Il2CppGetClassType(const char *image, const char *namespaze, const char *clazz) {
	...
	__android_log_print(ANDROID_LOG_DEBUG, g_LogTag, "Klass: %p", klass);
	if (!klass) {
...
size_t Il2CppGetFieldOffset(const char *image, const char *namespaze, const char *clazz, const char *name) {
	...
	void *klass = Il2CppGetClassType(image, namespaze, clazz);
	if (!klass) {
    __android_log_print(ANDROID_LOG_ERROR, g_LogTag, "Can't find class %s::%s!", namespaze, clazz);
    return -1;
}
	void *field = il2cpp_class_get_field_from_name(klass, name);
	if (!field) {
    __android_log_print(ANDROID_LOG_ERROR, g_LogTag, "Can't find field %s in class %s::%s!", name, namespaze, clazz);
    return -1;
}
...
bool Il2CppAttach(const char *name) {
	il2cpp_domain_get = (void *(*)()) Tools::GetRealOffsets(name, IL2CPP_DOMAIN_GET);
	__android_log_print(ANDROID_LOG_DEBUG, g_LogTag, "IL2CPP_DOMAIN_GET: %p", il2cpp_domain_get);
	il2cpp_domain_get_assemblies = (void **(*)(const void* , size_t*)) Tools::GetRealOffsets(name, IL2CPP_DOMAIN_GET_ASSEMBLIES);
	__android_log_print(ANDROID_LOG_DEBUG, g_LogTag, "IL2CPP_DOMAIN_GET_ASSEMBLIES: %p", il2cpp_domain_get_assemblies);
	il2cpp_assembly_get_image = (void *(*)(void *assembly)) Tools::GetRealOffsets(name, IL2CPP_ASSEMBLY_GET_IMAGE);
	__android_log_print(ANDROID_LOG_DEBUG, g_LogTag, "IL2CPP_ASSEMBLY_GET_IMAGE: %p", il2cpp_assembly_get_image);
	il2cpp_domain_assembly_open = (void *(*)(void *domain, const char *name)) Tools::GetRealOffsets(name, IL2CPP_DOMAIN_ASSEMBLY_OPEN);
	__android_log_print(ANDROID_LOG_DEBUG, g_LogTag, "IL2CPP_DOMAIN_ASSEMBLY_OPEN: %p", il2cpp_domain_assembly_open);
	il2cpp_class_from_name = (void* (*)(const void*, const char*, const char *)) Tools::GetRealOffsets(name, IL2CPP_CLASS_FROM_NAME);
	__android_log_print(ANDROID_LOG_DEBUG, g_LogTag, "IL2CPP_CLASS_FROM_NAME: %p", il2cpp_class_from_name);
	il2cpp_class_get_field_from_name = (void* (*)(void*, const char *)) Tools::GetRealOffsets(name, IL2CPP_CLASS_GET_FIELD_FROM_NAME);
	__android_log_print(ANDROID_LOG_DEBUG, g_LogTag, "IL2CPP_CLASS_GET_FIELD_FROM_NAME: %p", il2cpp_class_get_field_from_name);
	il2cpp_class_get_method_from_name = (void* (*)(void *, const char*, int)) Tools::GetRealOffsets(name, IL2CPP_CLASS_GET_METHOD_FROM_NAME);
	__android_log_print(ANDROID_LOG_DEBUG, g_LogTag, "IL2CPP_CLASS_GET_METHOD_FROM_NAME: %p", il2cpp_class_get_method_from_name);
